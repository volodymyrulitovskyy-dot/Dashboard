...
export async function POST(request: Request) {
  ...
  const rateLimit = checkRateLimit(
    `api:workflows:labor-distribution:scheduled:${sourceIp}`,
    {
      maxRequests: 30,
      windowMs: 60 * 1000,
    },
  ...
  if (!isAuthorizedRequest(request)) {
    logger.warn({ sourceIp }, "scheduled labor workflow unauthorized request");
    return NextResponse.json(
      { error: "Unauthorized" },
  ...
  const query = querySchema.safeParse({
    ...
    allowPartialAllocation: url.searchParams.get("allowPartialAllocation") ?? undefined,
  });
  ...
  try {
    ...
    const results = await Promise.all(
      teamIds.map((teamId) =>
        runLaborCostDistributionWorkflow({
          teamId,
          periodKey: query.data.periodKey,
          trigger: "scheduled",
          executedByUserId: "system-scheduler",
          dryRun: query.data.dryRun,
          allowPartialAllocation: query.data.allowPartialAllocation,
        }).catch((error: unknown) => ({
          teamId,
          error:
    ...
    logger.error({ error, sourceIp }, "scheduled labor workflow execution failed");
    return NextResponse.json(
